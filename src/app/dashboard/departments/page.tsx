...
const TeamsDashboard = () => {
  ...
  const getColumns = useCallback((): ColumnDef<DepartmentData>[] => {
    ...
    const monthColumns: ColumnDef<DepartmentData>[] = sortedSelectedMonths.map(
      (month) => ({
        id: month,
        header: month,
        columns: [
            ...
            ? [
                {
                  ...
                  accessorFn: (row: DepartmentData) => {
                    const monthData = row.monthData.find(
                      (md) => md.month === month
                    );
                    return monthData
                      ? Number(monthData.avgTotalCallMinutes.replace(/,/g, ""))
                      : 0;
                  },
                  header: ({ column }) => (
                    <div className="text-center dark:text-white">
                      <SortableHeader
                        column={column}
                        title={`Avg Total Call Minutes - ${month}`}
                      />
                    </div>
                  ),
                  cell: ({ getValue }) => (
            ...
            ? [
                {
                  ...
                  accessorFn: (row: DepartmentData) => {
                    ...
                    return monthData
                      ? parseFloat(monthData.avgCallEfficiency) / 100
                      : 0;
                  },
                  header: ({ column }) => (
                    <div className="text-center dark:text-white">
                      <SortableHeader
                        column={column}
                        title={`Avg Call Efficiency - ${month}`}
                      />
                    </div>
                  ),
                  cell: ({ getValue }) => (
                    <div className="text-center">
                      {formatDecimal(getValue() as number)}
                    </div>
                  ),
                },
              ]
            ...
            ? [
                {
                  ...
                  accessorFn: (row: DepartmentData) => {
                    ...
                    return monthData
                      ? Number(monthData.avgTotalSales.replace(/,/g, ""))
                      : 0;
                  },
                  header: ({ column }) => (
                    <div className="text-center dark:text-white">
            ...
            ? [
                {
                  ...
                  accessorFn: (row: DepartmentData) => {
                    ...
                    return monthData
                      ? parseFloat(monthData.avgRatioBetweenSkadeAndLiv) / 100
                      : 0;
                  },
                  header: ({ column }) => (
                    <div className="text-center dark:text-white">
                      <SortableHeader
                        column={column}
                        title={`Avg Ratio Between Skade & Liv - ${month}`}
                      />
                    </div>
                  ),
                  cell: ({ getValue }) => (
            ...
            ? [
                {
                  ...
                  accessorFn: (row: DepartmentData) => {
                    ...
                    return monthData?.avgTotalScore ?? 0;
                  },
                  header: ({ column }) => (
                    <div className="text-center dark:text-white">
                      <SortableHeader
                        column={column}
                        title={`Avg Total Score - ${month}`}
                      />
                    </div>
                  ),
                  cell: ({ getValue }) => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/merged-names-departments?${params.toString()}`
      );
  ...
  const transformApiResponse = (apiData: any[]): DepartmentData[] => {
    ...
    apiData.forEach((item) => {
      ...
      const monthData = {
        month: item.month,
        avgTotalCallMinutes: item.avgTotalCallMinutes,
        tcmScore: item.tcmScore,
        avgCallEfficiency: item.avgCallEfficiency,
        ceScore: item.ceScore,
        avgTotalSales: item.avgTotalSales,
        tsScore: item.tsScore,
        avgRatioBetweenSkadeAndLiv: item.avgRatioBetweenSkadeAndLiv,
