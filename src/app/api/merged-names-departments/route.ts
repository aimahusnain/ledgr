// /api/merged-names-departments/route.ts
  ...
  try {
    ...
    const monthlyData = await Promise.all(
      months.map(async (month) => {
        ...
        const individualDataMap = new Map<string, { totalCallMinutes: number; totalOutgoingCalls: number; totalSales: number; livSales: number; department: string }>();

        incomingCalls.forEach((call) => {
          const normalizedName = normalizeAndTrim(call.navn);
          if (normalizedName) {
            const data = individualDataMap.get(normalizedName) || { totalCallMinutes: 0, totalOutgoingCalls: 0, totalSales: 0, livSales: 0, department: "" };
        ...
        const departmentDetails: DepartmentDetails[] = Array.from(departmentDetailsMap.keys())
          .map((department) => {
